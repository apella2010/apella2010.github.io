...
let AppComponent = class AppComponent {
    constructor(prodService, authService) {
        this.prodService = prodService;
        ...
        this.catItems = null;
        this.category = null;
        this.prodService.catItems.subscribe(res => {
            if (res != null) {
                this.catItems = res;
                this.category = res;
                this.prods = this.clone;
                this.prods = this.prods.filter(item => { return item.attributes.category.toLowerCase() == this.catItems.toLowerCase(); });
            }
            else {
                this.prods = this.clone;
            }
        ...
        let self = this;
        this.prodService.cats.subscribe(res => {
            this.cats = res;
        });
        this.prodService.prod.subscribe(res => this.prod = res);
        this.authService.fake.subscribe(res => this.fake = res);
        this.prodService.bucket.subscribe(res => {
            if (res != null) {
                ...
                this.bucket_numb = 0;
                this.prodArr = [];
                if (this.bucket.product_id && Object.keys(this.bucket.product_id).length > 0) {
                    let keys = Object.keys(this.bucket.product_id);
                    keys.forEach(el => {
                        let prod = this.prods.filter(elem => { return elem.id == el; });
        ...
        this.prodService.news.subscribe(res => {
            this.news = res;
        });
        this.authService.validateToken().subscribe((res) => {
            if (res.success) {
                // this.authService.userSignedIn$.next(true);
                this.authService.user.next(this.authService.userData());
                if (this.authService.userType() == "ADMIN") {
                    ...
                    this.authService.user.next(this.user);
                    this.authService.fake.next(null);
                    this.prodService.getBucket(this.user.id).subscribe(res => {
                        this.prodService.bucket.next(res);
                    });
                }
            }
        }, (err) => {
            if (localStorage['ognetushitely_user_id']) {
                this.fake = { id: Number(localStorage['ognetushitely_user_id']) };
                this.prodService.getBucket(this.fake.id).subscribe(res => {
                    if (res["id"]) {
                        this.prodService.bucket.next(res);
                    }
                    else {
                ...
                this.prodService.newBucket().subscribe(res => {
                    this.fake = { id: res };
                    localStorage['ognetushitely_user_id'] = res;
                    this.prodService.getBucket(this.fake.id).subscribe(res => {
                        this.prodService.bucket.next(res);
        ...
        this.prodService.getProds().subscribe(res => {
            this.bla = true;
            this.prodService.cats.next(res.cats);
            this.prods = JSON.parse(res.prods).data;
            this.prodService.getNews().subscribe(res => this.prodService.news.next(res));
            this.prodService.countDB = Math.ceil(res.count / 50);
            this.count = res.count;
            for (let i = 2; i < Math.ceil(res.count / 50) + 1; i++) {
                this.prodService.getFirst(i).subscribe(res => {
...
/* harmony import */ var angular2_draggable__WEBPACK_IMPORTED_MODULE_18__ = __webpack_require__(/*! angular2-draggable */ "./node_modules/angular2-draggable/fesm2015/angular2-draggable.js");
...
/* harmony import */ var angular_token__WEBPACK_IMPORTED_MODULE_23__ = __webpack_require__(/*! angular-token */ "./node_modules/angular-token/fesm2015/angular-token.js");
...
/* harmony import */ var ngx_virtual_scroller__WEBPACK_IMPORTED_MODULE_43__ = __webpack_require__(/*! ngx-virtual-scroller */ "./node_modules/ngx-virtual-scroller/fesm2015/ngx-virtual-scroller.js");
...
let BucketComponent = class BucketComponent {
    constructor(prodService, auth, router, orderService, parent) {
        this.prodService = prodService;
        this.auth = auth;
        this.router = router;
    ...
    addToBucket(prod_id) {
        ...
        this.prodService.addToBucket(this.parent.bucket).subscribe(res => {
            this.prodService.bucket.next(res);
        });
    }
    delFromBucket(prod_id) {
        if (this.parent.bucket.product_id[prod_id]) {
            this.parent.bucket.product_id[prod_id]--;
            this.prodService.addToBucket(this.parent.bucket).subscribe(res => {
                this.prodService.bucket.next(res);
...
BucketComponent.ctorParameters = () => [
    { type: _services_product_service__WEBPACK_IMPORTED_MODULE_2__["ProductService"] },
...
let ContactsComponent = class ContactsComponent {
    constructor(orderService, prodService) {
        this.orderService = orderService;
        this.prodService = prodService;
...
let EditorderComponent = class EditorderComponent {
    constructor(orderService, prodService, router, auth, parent) {
        this.orderService = orderService;
        this.prodService = prodService;
        this.router = router;
        this.auth = auth;
    ...
    ngOnInit() {
        ...
        this.auth.fake.subscribe(res => this.fake = res);
        this.orderService.order.subscribe(res => {
            this.order = res;
            this.prodArr = [];
            let keys = Object.keys(this.order.product_id);
            keys.forEach(el => {
                let prod = this.parent.prods.filter(elem => { return elem.id == el; });
                this.prodArr.push([prod[0], this.order.product_id[el]]);
...
let HomeComponent = class HomeComponent {
    constructor(prodService, router, auth) {
        this.prodService = prodService;
        this.router = router;
        this.auth = auth;
        ...
        this.news = [];
        this.user = null;
        this.prodService.news.subscribe(res => this.news = res);
        this.prodService.mda2 = false;
    ...
    deleteProd(prod) {
        if (window.confirm('Вы уверены что хотите удалить этот продукт?')) {
            ...
            prod.images.forEach(function (element) {
                let s3 = new self.AWS.S3().deleteObject({ Bucket: self.env.bucket, Key: element.split('/')[element.split('/').length - 2] + '/' + element.split('/')[element.split('/').length - 1] }, function (err, data) {
                });
            });
...
let ItemNumber = class ItemNumber {
...
let LoginFormComponent = class LoginFormComponent {
    constructor(authService, prodService, router) {
        this.authService = authService;
        this.prodService = prodService;
        this.router = router;
        this.signInUser = {
    ...
    onSignInSubmit() {
        ...
        this.authService.logInUser(this.signInUser).subscribe(res => {
            // console.log(res)
            if (res.status == 200) {
                ...
                if (this.authService.userType()) {
                    ...
                    this.prodService.getBucket(res['body']['data']['id']).subscribe(res => {
                        this.prodService.bucket.next(res);
                    });
                }
            }
...
let MainComponent = class MainComponent {
    constructor(prodService, auth, router, parent) {
        this.prodService = prodService;
        this.auth = auth;
        this.router = router;
        this.parent = parent;
    ...
    addToBucket(prod_id) {
        ...
        this.prodService.addToBucket(this.bucket).subscribe(res => {
            this.prodService.bucket.next(res);
        });
    }
};
...
class Mail {
    constructor(milo, name, tutle, text) {
        this.milo = milo;
        this.name = name;
        this.tutle = tutle;
        this.text = text;
...
class Order {
    constructor(number, product_id, adress, user_email, kontakt_phone, price, user_id, id, condition) {
        this.number = number;
...
class Product {
    constructor(id, type, attributes, title, images, description, amount, category, views, news, price) {
        this.id = id;
        this.type = type;
...
let OrderComponent = class OrderComponent {
    constructor(prodService, auth, orderService, router, parent) {
        this.prodService = prodService;
        this.auth = auth;
        this.orderService = orderService;
        this.router = router;
    ...
    createOrder(order) {
        ...
        this.orderService.createOrder(order).subscribe(res => {
            ...
            this.orderService.closeModal();
            this.prodService.cleanBucket(this.parent.user ? this.parent.user.id : this.parent.fake.id).subscribe();
            this.prodService.bucket.next([]);
            this.router.navigate(['/']);
        });
    }
};
...
let OrderdialogComponent = class OrderdialogComponent {
...
OrderdialogComponent.ctorParameters = () => [
    { type: _services_order_service__WEBPACK_IMPORTED_MODULE_2__["OrderService"] }
];
...
let OrdersComponent = class OrdersComponent {
    constructor(orderService, prodService, router, parent) {
        this.orderService = orderService;
        this.prodService = prodService;
        this.router = router;
        this.parent = parent;
        ...
        // this.prodService.prods.subscribe(res=> this.prods=res);
        this.orderService.orders.subscribe(res => this.orders = res);
        this.orderService.getOrders().subscribe(res => this.orderService.orders.next(res));
    }
    addOrd(order) {
        //  let self = this;
...
let ProddialogComponent = class ProddialogComponent {
    constructor(prodService) {
        this.prodService = prodService;
    }
    onNewFormResult(e) {
...
ProddialogComponent.ctorParameters = () => [
    { type: _services_product_service__WEBPACK_IMPORTED_MODULE_2__["ProductService"] }
];
...
let ProducteditComponent = class ProducteditComponent {
    constructor(prodService, pic, auth) {
        this.prodService = prodService;
        this.pic = pic;
        this.auth = auth;
    ...
    update(prod) {
        ...
        this.prodService.updateProd(prod).subscribe(res => {
            // console.log(res);
            this.prodService.prod.next(res);
            if (res) {
                this.onFormResult.emit({ signedIn: true, res });
            }
        }, err => {
    ...
    fileEvent(data) {
        ...
        let self = this;
        let params = { Bucket: this.env.bucket, Key: this.prod.id + '/' + this.makeid(), Body: data };
        ...
        s3.on('httpUploadProgress', function (evt) {
            ...
            if (error) {
                ...
                self.prodService.updateProd(self.prod).subscribe(res => {
    ...
    delImg(idx) {
        let self = this;
        let s3 = new this.AWS.S3().deleteObject({ Bucket: this.env.bucket, Key: this.prod.id + '/' + this.prod.images[idx].split('/')[this.prod.images[idx].split('/').length - 1] }, function (err, data) {
            if (data) {
...
/* harmony import */ var ngx_virtual_scroller__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ngx-virtual-scroller */ "./node_modules/ngx-virtual-scroller/fesm2015/ngx-virtual-scroller.js");
...
let ProductlistComponent = class ProductlistComponent {
    // cardHeight:number;
    constructor(parent, prodService, router, auth) {
        this.parent = parent;
        this.prodService = prodService;
        this.router = router;
        this.auth = auth;
    ...
    ngOnInit() {
        window.addEventListener('scroll', (el) => {
            this.scrollPos = el['target']['scrollingElement']['scrollTop'];
        ...
        if (localStorage['search']) {
            ...
            this.parent.prods = this.parent.prods.filter(item => item.attributes.title.toLowerCase().indexOf(localStorage['search'].toLowerCase()) > -1);
    ...
    filterItem(value) {
        ...
        if (!value) {
            ...
            this.parent.prods = this.parent.prods.filter(item => item.attributes.title.toLowerCase().indexOf(value.toLowerCase()) > -1);
        }
    }
    gooUp() {
        window.scrollTo(0, 0);
...
let ProductnewComponent = class ProductnewComponent {
    ...
    createProd(prod) {
        ...
        this.prodService.createProd(prod).subscribe(res => {
            if (res) {
                this.onFormResult.emit({ signedIn: true, res });
                this.urls = [];
                this.sucess = false;
                ...
                res.images.forEach(function (el) {
                    let s3 = new self.AWS.S3().copyObject({ Bucket: self.env.bucket + '/' + res.id, CopySource: self.env.bucket + '/' + el.split('/')[el.split('/').length - 1], Key: el.split('/')[el.split('/').length - 1] }, function (err, data) {
                        let s3 = new self.AWS.S3().deleteObject({ Bucket: self.env.bucket, Key: el.split('/')[el.split('/').length - 1] }, function (err, data) {
                        });
                    });
                });
            }
    ...
    fileEvent(data) {
        ...
        this.img_upload = true;
        let self = this;
        let params = { Bucket: this.env.bucket, Key: this.makeid(), Body: data };
        let s3 = new this.AWS.S3.ManagedUpload({ params: params });
        s3.on('httpUploadProgress', function (evt) {
    ...
    getFile2(fileInput) {
        ...
        this.sucess = false;
        this.img_upload = true;
        let params = { Bucket: this.env.bucket, Key: 'price.xlsx', Body: file };
        let s3 = new self.AWS.S3().deleteObject({ Bucket: self.env.bucket, Key: "price.xlsx" }, function (err, data) {
            let s3 = new self.AWS.S3.ManagedUpload({ params: params });
            s3.on('httpUploadProgress', function (evt) {
                jquery__WEBPACK_IMPORTED_MODULE_6__('#pus').css('width', '0%');
                jquery__WEBPACK_IMPORTED_MODULE_6__('#pus').css('width', evt.loaded * 100 / evt.total + '%');
            }).send(function (error, s3res) {
    ...
    delFromUpl(idx) {
        let self = this;
        let s3 = new this.AWS.S3().deleteObject({ Bucket: this.env.bucket, Key: this.urls[idx].split('/')[this.urls[idx].split('/').length - 1] }, function (err, data) {
            if (data) {
                ;
...
let ProductshowComponent = class ProductshowComponent {
    constructor(prodService, route, router, auth, parent) {
        this.prodService = prodService;
        this.route = route;
        this.router = router;
        this.auth = auth;
    ...
    ngOnInit() {
        ...
        this.route.params.subscribe(params => {
            ...
            this.prodService.getProd(this.id).subscribe(res => {
                this.prodService.prod.next(res['data']['attributes']);
            });
        });
        this.env.bucket = config__WEBPACK_IMPORTED_MODULE_6__["keys"].aws_bucket;
        this.env.id = config__WEBPACK_IMPORTED_MODULE_6__["keys"].aws_id;
...
ProductshowComponent.ctorParameters = () => [
    { type: _services_product_service__WEBPACK_IMPORTED_MODULE_3__["ProductService"] },
...
let ProfileComponent = class ProfileComponent {
    constructor(authService, router, prodService, parent) {
        this.authService = authService;
        this.router = router;
        this.prodService = prodService;
        this.parent = parent;
    ...
    deleteUser() {
        ...
        if (window.confirm('Вы уверены, что хотите удалить аккаунт?')) {
            ...
            this.authService.deleteUser().subscribe(() => {
                if (localStorage['ognetushitely_user_id']) {
                    this.authService.fake.next({ id: Number(localStorage['ognetushitely_user_id']) });
                    this.prodService.getBucket(localStorage['ognetushitely_user_id']).subscribe(res => {
                        this.prodService.bucket.next(res);
                    });
                }
                else {
                    this.prodService.newBucket().subscribe(res => {
                        this.authService.fake.next({ id: res.user_id });
...
let RegisterFormComponent = class RegisterFormComponent {
    ...
    onSignUpSubmit() {
        ...
        this.authService.registerUser(this.signUpUser).subscribe((res) => {
            if (res.status == 200) {
                this.onFormResult.emit({ signedUp: true, res });
                this.authService.user.next(this.authService.userData());
                this.authService.fake.next(null);
                this.router.navigate(['/']);
                this.prodService.createBucket(res['body']["data"]["id"]).subscribe(res => { this.prodService.bucket.next(res); });
            }
        }, (err) => {
            console.log(err);
...
let AuthGuard = class AuthGuard {
    constructor(authTokenService, router) {
        this.authTokenService = authTokenService;
        this.router = router;
    }
    canActivate() {
        return this.authTokenService.validateToken().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(res => {
            if (res['success']) {
                if (this.authTokenService.userType() == 'ADMIN') {
                    return true;
                }
                else {
                    this.router.navigate(['/']);
...
/* harmony import */ var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! tslib */ "./node_modules/tslib/tslib.es6.js");
/* harmony import */ var _angular_core__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @angular/core */ "./node_modules/@angular/core/fesm2015/core.js");
/* harmony import */ var angular_token__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! angular-token */ "./node_modules/angular-token/fesm2015/angular-token.js");
...
let AuthService = class AuthService {
    constructor(authService, router, prod) {
        this.authService = authService;
        this.router = router;
        this.prod = prod;
...
let AuthCheck2 = class AuthCheck2 {
    constructor(authTokenService, router) {
        this.authTokenService = authTokenService;
        this.router = router;
    }
...
let ProductService = class ProductService {
    ...
    createProd(prod) {
        ...
        return this.http.post(this.prodsUrl, JSON.stringify(prod), { headers: this.headers })
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])((res) => res["data"]["attributes"]));
    }
    getProd(id) {
    ...
    updateProd(prod) {
        ...
        return this.http.put(url, JSON.stringify(prod), { headers: this.headers }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])((res) => res["data"]["attributes"]));
    }
    openModal(mode = 'new') {
        this.modeDial = mode;
        this.modalActions.emit({ action: "modal", params: ['open'] });
    ...
    getBucket(user_id) {
        return this.http.get(this.mainUrl + 'buckets/' + user_id).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])((response) => response));
    }
    newBucket() {
    ...
    smdispbucket(prod) {
        ...
        window.setTimeout(() => {
            this.shprod.next(null);
        }, 4000);
    }
    isNewMode() { return this.modeDial == 'new'; }
    isEditMode() { return this.modeDial == 'edit'; }
};
...
let ToolbarComponent = class ToolbarComponent {
    constructor(orderService, authService, router, prodService, parent) {
        this.orderService = orderService;
        this.authService = authService;
        this.router = router;
        this.prodService = prodService;
        ...
        this.shprod = null;
        this.prodService.mda = false;
        this.prodService.shprod.subscribe(res => {
            this.shprod = res;
            // console.log(res)
        });
    }
    ...
    logOut() {
        ...
        this.authService.logOutUser().subscribe(() => {
            if (this.prodService.mda) {
                this.router.navigate(['/']);
            }
            ;
            this.prodService.bucket.next(null);
            this.authService.user.next(null);
            if (localStorage['ognetushitely_user_id']) {
                this.parent.fake = { id: Number(localStorage['ognetushitely_user_id']) };
                this.authService.fake.next(this.parent.fake);
                this.prodService.getBucket(localStorage['ognetushitely_user_id']).subscribe(res => {
                    if (res["id"]) {
                        this.prodService.bucket.next(res);
